...
for group in $(seq 0 $((groups-1))); do
    ...
    echo "do" >> $script
    echo "    python3 download.py --split_dir /mnt/data/objarverse_df_splits --savedir $SAVEDIR --filetype blend --source github --chunk \$chunk" >> $script
    echo "done" >> $script
